...
class AllNotificationsList(NotificationViewList):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["title"] = "通知"
        context["site_url"] = SimpleLazyObject(lambda: "{0}://{1}".format('https' if self.request.is_secure() else 'http', SimpleLazyObject(lambda: get_current_site(self.request))))
        context["unread"] = self.request.user.notifications.unread()
...
class UnreadNotificationsList(NotificationViewList):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["title"] = "通知"
        context["site_url"] = SimpleLazyObject(lambda: "{0}://{1}".format('https' if self.request.is_secure() else 'http', SimpleLazyObject(lambda: get_current_site(self.request))))
        context["notifications_all"] = self.request.user.notifications.all()
        context["unread"] = self.request.user.notifications.unread()
        context["active_unread"] = True
        return context
